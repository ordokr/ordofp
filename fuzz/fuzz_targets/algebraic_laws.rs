...
fuzz_target!(
    |data: (Vec<i8>, Vec<i8>, Vec<i8>, String, String, Option<i64>)| {
        ...
        assert!(semigroup_laws::associativity(&a, &b, &c));
        assert!(monoid_laws::left_identity(&a));
        assert!(monoid_laws::right_identity(&a));
        assert!(semigroup_laws::associativity(&s1, &s2, &String::new()));
