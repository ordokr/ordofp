...
impl<T: Ord + Clone> Compositio for Max<T> {
    ...
    fn combine(&self, other: &Self) -> Self {
        match self.0.cmp(&other.0) {
            Ordering::Less => Self(other.0.clone()),
            _ => Self(self.0.clone()),
        }
    }
}
...
impl<T: Ord + Clone> Compositio for Min<T> {
    ...
    fn combine(&self, other: &Self) -> Self {
        match self.0.cmp(&other.0) {
            Ordering::Less => Self(self.0.clone()),
            _ => Self(other.0.clone()),
        }
    }
}
...
impl<T: Compositio> Compositio for Box<T> {
    #[inline]
    fn combine(&self, other: &Self) -> Self {
        Self::new(self.deref().combine(&**other))
    }
}
...
impl<T: Compositio + Copy> Compositio for Cell<T> {
    #[inline]
    fn combine(&self, other: &Self) -> Self {
        Self::new(self.get().combine(&other.get()))
    }
}
...
impl<T: Compositio> Compositio for RefCell<T> {
    #[inline]
    fn combine(&self, other: &Self) -> Self {
        Self::new(self.borrow().deref().combine(other.borrow().deref()))
    }
}

#[cfg(feature = "std")]
impl<T: Eq + Hash + Clone, S: BuildHasher + Default> Compositio for HashSet<T, S> {
    #[inline]
    fn combine(&self, other: &Self) -> Self {
...
#[cfg(feature = "std")]
impl<K: Eq + Hash + Clone, V: Compositio + Clone, S: BuildHasher + Clone> Compositio
    for HashMap<K, V, S>
{
    #[inline]
    fn combine(&self, other: &Self) -> Self {
...
pub fn combine_n<T: Compositio + Clone>(o: &T, times: u32) -> T {
    ...
    while n > 0 {
        if n % 2 == 1 {
            acc = acc.map_or_else(|| Some(base.clone()), |a| Some(a.combine(&base)));
        }
        if n > 1 {
...
mod tests {
    ...
    fn test_numeric() {
        ...
        assert_eq!(1.5f64.combine(&2.5).to_bits(), 4.0f64.to_bits());
    }
    ...
    #[test]
    fn test_numeric_f32() {
        assert_eq!(1f32.combine(&2f32).to_bits(), 3f32.to_bits());
    }
    ...
    fn test_numeric_f64_integers() {
        assert_eq!(1f64.combine(&2f64).to_bits(), 3f64.to_bits());
    }
